...
class MainWindow(QMainWindow):
    ...
    def initPrelucrare(self):        
        ...
        index = self.comboBox_tipFereastraFIR.findText('kaiser', Qt.MatchFixedString)
        self.comboBox_tipFereastraFIR.setCurrentIndex(index)
        ...
        index = self.comboBox_FiltruIIR.findText('Eliptic', Qt.MatchFixedString)
        self.comboBox_FiltruIIR.setCurrentIndex(index)
        index = self.comboBox_tipFiltruIIR.findText('FTB', Qt.MatchFixedString)
        self.comboBox_tipFiltruIIR.setCurrentIndex(index)
        ...
        self.lineEdit_dimensiuneFereastra.setText('5')
        self.pushButton_aplicaMediere.clicked.connect(self.applySettingsAndPlotMean)
    
    def initAfisare(self):
        self.processedData = {'x': [None]*self.numberOfChannels, 'y': [None]*self.numberOfChannels}
        self.autocorrelationData = [None]*self.numberOfChannels
        
        self.settings = [None] * self.numberOfChannels
        
        self.processingThread = Processing(self.dataRead, self.processedData, self.autocorrelationData, self.settings, self.numberOfChannels)
        self.processingThread.start()
        
        # Lista widget-urilor de tip plot din interfata
        self.pltWidgets = [self.plotWidget_channel_1, self.plotWidget_channel_2,
        ...
        self.plotWidget_autocorrelation = [self.plotWidget_autocorrelation_channel_1,
                                           self.plotWidget_autocorrelation_channel_2,
                                           self.plotWidget_autocorrelation_channel_3,
                                           self.plotWidget_autocorrelation_channel_4,
                                           self.plotWidget_autocorrelation_channel_5,
                                           self.plotWidget_autocorrelation_channel_6,
                                           self.plotWidget_autocorrelation_channel_7,
                                           self.plotWidget_autocorrelation_channel_8]
        
        for channel in range(self.numberOfChannels):
            self.plotWidget_autocorrelation[channel].close()
        
        # Pornirea thread-ului care ploteaza
        self.plots = Plot(self.processedData, self.autocorrelationData, self.numberOfChannels, self.pltWidgets, self.plotWidget_autocorrelation, self.settings)
        self.plots.start()
        ...
        self.meanButtons = [self.pushButton_Mediere_1, self.pushButton_Mediere_2,
                            self.pushButton_Mediere_3, self.pushButton_Mediere_4,
                            self.pushButton_Mediere_5, self.pushButton_Mediere_6,
                            self.pushButton_Mediere_7, self.pushButton_Mediere_8]
        
        self.autocorrelationButtons = [self.pushButton_Autocorelatie_1,
                                       self.pushButton_Autocorelatie_2,
                                       self.pushButton_Autocorelatie_3,
                                       self.pushButton_Autocorelatie_4,
                                       self.pushButton_Autocorelatie_5,
                                       self.pushButton_Autocorelatie_6,
                                       self.pushButton_Autocorelatie_7,
                                       self.pushButton_Autocorelatie_8]
        ...
        self.pushButton_Mediere_1.clicked.connect(lambda: self.meanSetting(0))
        self.pushButton_Mediere_2.clicked.connect(lambda: self.meanSetting(1))
        self.pushButton_Mediere_3.clicked.connect(lambda: self.meanSetting(2))
        self.pushButton_Mediere_4.clicked.connect(lambda: self.meanSetting(3))
        self.pushButton_Mediere_5.clicked.connect(lambda: self.meanSetting(4))
        self.pushButton_Mediere_6.clicked.connect(lambda: self.meanSetting(5))
        self.pushButton_Mediere_7.clicked.connect(lambda: self.meanSetting(6))
        self.pushButton_Mediere_8.clicked.connect(lambda: self.meanSetting(7))
        
        self.pushButton_Autocorelatie_1.clicked.connect(lambda: self.autocorrelationSetting(0))
        self.pushButton_Autocorelatie_2.clicked.connect(lambda: self.autocorrelationSetting(1))
        self.pushButton_Autocorelatie_3.clicked.connect(lambda: self.autocorrelationSetting(2))
        self.pushButton_Autocorelatie_4.clicked.connect(lambda: self.autocorrelationSetting(3))
        self.pushButton_Autocorelatie_5.clicked.connect(lambda: self.autocorrelationSetting(4))
        self.pushButton_Autocorelatie_6.clicked.connect(lambda: self.autocorrelationSetting(5))
        self.pushButton_Autocorelatie_7.clicked.connect(lambda: self.autocorrelationSetting(6))
        self.pushButton_Autocorelatie_8.clicked.connect(lambda: self.autocorrelationSetting(7))
    ...
    def initInterpretareGesturi(self):
        ...
        self.NeuralNetwork.doneClassification.connect(self.gestureIdentificationDone)
    ...
    def readyWindows(self, window):
        arg = [self.plotFIR.filter_coeffs.tolist(), self.plotIIR.a.tolist(), self.plotIIR.b.tolist(), self.plotMediere.sizeWindow]
        ...
        if self.classificationFlag == True:
            ...
            nnSelected = self.comboBox_NN.currentText()
            arg = [window, nnSelected]
            
            if nnSelected in ['NN - 5 gesturi - caracteristici in timp - TensorFlow',
                            'NN - 5 gesturi - caracteristici in timp - Versiunea mea',
                            'NN - 15 gesturi - caracteristici in timp - TensorFlow',
                            'NN - 15 gesturi - caracteristici in timp - Versiunea mea']:
                QMetaObject.invokeMethod(self.NeuralNetwork, 'classificationTime', Qt.QueuedConnection, Q_ARG(list, arg))
            else:
                QMetaObject.invokeMethod(self.NeuralNetwork, 'classificationFFT', Qt.QueuedConnection, Q_ARG(list, arg))
    
    def fftAbsSetting(self, channel):
        if self.fftAbsButtons[channel].isChecked():
            self.plotWidget_autocorrelation[channel].close()
            self.pltWidgets[channel].show()
            ...
            self.meanButtons[channel].setChecked(False)
            self.meanButtons[channel].setStyleSheet("background-color: white;")
            self.autocorrelationButtons[channel].setChecked(False)
            self.autocorrelationButtons[channel].setStyleSheet("background-color: white;")
    ...
    def meanSetting(self, channel):
        if self.meanButtons[channel].isChecked():
            self.plotWidget_autocorrelation[channel].close()
            self.pltWidgets[channel].show()
            
            self.settings[channel] = 'mean'
            ...
            self.meanButtons[channel].setStyleSheet("background-color: lightgray;")
        else:
            self.settings[channel] = None
            self.meanButtons[channel].setStyleSheet("background-color: white;")
    
    def autocorrelationSetting(self, channel):
        if self.autocorrelationButtons[channel].isChecked():
            self.pltWidgets[channel].close()
            self.plotWidget_autocorrelation[channel].show()
            
            self.settings[channel] = 'autocorrelation'
            ...
            self.autocorrelationButtons[channel].setStyleSheet("background-color: lightgray;")
        else:
            self.plotWidget_autocorrelation[channel].close()
            self.pltWidgets[channel].show()
    ...
    def classificationDone(self):
        self.classificationFlag = True
    
    def gestureIdentificationDone(self, classification):
    ...
    def applySettingsAndPlotMean(self):
        self.plotMediere.applyNewSettings(int(self.lineEdit_dimensiuneFereastra.text()))
